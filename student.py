...
class Student:
    def __init__(self, root):
        ...
        img_left = img_left.resize((760, 150), Image.ANTIALIAS)
        ...
        dep_combo["values"] = (
            ...
            "Electrical Engineering","Electronics & Telecommunication Engineering", "Mechatronics")
        dep_combo.current(0)
        ...
        class_student_Frame = LabelFrame(Left_Frame, bd=2, bg="white", relief=RIDGE, text="Class Student Information",
                                         font=("times new roman", 12, "bold"))
        class_student_Frame.place(x=5, y=265, width=760, height=355)

        # Student ID
        stud_id_label = Label(class_student_Frame, text="Enrollment No :", font=("times new roman", 13, "bold"),
                              bg="white")
        ...
        btn_take_photo = Button(btn_photo_Frame, command=self.generate_dataset, text="Take Photo Sample", width=37,
                                font=("times new roman", 13, "bold"), bg="blue", fg="white", bd=4)
        btn_take_photo.grid(row=-0, column=0)

        btn_update_photo = Button(btn_photo_Frame, text="Update Photo Sample", width=37,
                                  font=("times new roman", 13, "bold"), bg="blue", fg="white", bd=4)
        btn_update_photo.grid(row=0, column=1)
        ...
        img_right = Image.open(r"college_images\student2.jpg")
        img_right = img_right.resize((760, 150), Image.ANTIALIAS)
        ...
        search_Frame = LabelFrame(Right_Frame, bd=2, bg="white", relief=RIDGE, text="Search System",
                                  font=("times new roman", 12, "bold"))
        search_Frame.place(x=5, y=135, width=759, height=70)
        ...
        btn_search = Button(search_Frame, text="Search", width=15, font=("times new roman", 13, "bold"), bg="blue",
                            fg="white", bd=4)
        btn_search.grid(row=0, column=3, padx=4)

        btn_showAll = Button(search_Frame, text="Show All", width=15, font=("times new roman", 13, "bold"), bg="blue",
                             fg="white", bd=4)
        ...
        table_Frame.place(x=5, y=210, width=758, height=415)
    ...
    def add_data(self):
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="BlueBerry@14",
        ...
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute(
                        "update student set Dep=%s, Course=%s, Year=%s, Sem=%s, Name=%s, Division=%s, Roll=%s, "
        ...
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                ...
                for x in myresult:
                    id += 1
                my_cursor.execute(
                    "update student set Dep=%s, Course=%s, Year=%s, Sem=%s, Name=%s, Division=%s, Roll=%s, Gender=%s,"
